    ...
    <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-lg border-b border-accent/30 shadow-lg' : 'bg-white/90 backdrop-blur-md border-b border-accent/20 shadow-sm'}`}>
      <div className="container mx-auto px-4 py-4 flex items-center justify-between gap-6">
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 bg-gradient-to-br from-brand to-brand/80 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md">BL</div>
        ...
        <button className="md:hidden p-2 rounded-lg hover:bg-brand/10 transition-colors duration-200" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label="Toggle mobile menu" aria-expanded={isMobileMenuOpen}>
        ...
        <Link href="/cart" className="btn btn-brand relative group" aria-label={`View cart with ${count} items`}>
          <span className="flex items-center gap-2">
            <div className="relative">
              ...
              {count > 0 && <div className="absolute -top-2 -right-2 w-5 h-5 bg-accent rounded-full flex items-center justify-center text-xs font-bold text-white">{count}</div>}
            </div>
